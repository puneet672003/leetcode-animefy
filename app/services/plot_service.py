...
def _calculate_battle_stats(
    users: list[UserProgress], improvement_map: dict[str, int] = None
) -> dict:
    scores = []
    for user in users:
        ...
        scores.append(
            {"user": user.username, "total": total_solved, "improvement": improvement}
        )

    sorted_scores = sorted(
        scores, key=lambda x: (x["improvement"], x["total"]), reverse=True
    )
...
async def generate_scene(
    ...
    improvement_map: dict[str, int] = None,
    prompt_name: str = "intro",
) -> dict:
